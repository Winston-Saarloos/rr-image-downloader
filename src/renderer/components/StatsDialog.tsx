...
interface PhotoStats {
  ...
  totalCheers: number;
  largestTimeGap: { days: number; hours: number; minutes: number; photo1Date?: Date; photo2Date?: Date } | null;
  smallestTimeGap: { days: number; hours: number; minutes: number; photo1Date?: Date; photo2Date?: Date } | null;
...
export const StatsDialog: React.FC<StatsDialogProps> = ({
  ...
  const stats = useMemo((): PhotoStats => {
    ...
    const photosWithDates = allPhotos
      ...
      .filter((item): item is { photo: Photo; date: Date } => item.date !== null)
      .sort((a, b) => a.date.getTime() - b.date.getTime());

    // Calculate time gaps
    let largestGap: { days: number; hours: number; minutes: number; photo1Date?: Date; photo2Date?: Date } | null = null;
    let smallestGap: { days: number; hours: number; minutes: number; photo1Date?: Date; photo2Date?: Date } | null = null;
    ...
    for (let i = 0; i < photosWithDates.length - 1; i++) {
      ...
      const gap = { days, hours, minutes, photo1Date: date1, photo2Date: date2 };

      if (!largestGap || diffMs > (largestGap.days * 24 * 60 * 60 * 1000 + largestGap.hours * 60 * 60 * 1000 + largestGap.minutes * 60 * 1000)) {
        largestGap = gap;
      }
      if (!smallestGap || diffMs < (smallestGap.days * 24 * 60 * 60 * 1000 + smallestGap.hours * 60 * 60 * 1000 + smallestGap.minutes * 60 * 1000)) {
        smallestGap = gap;
      }
    }

    // First and latest photo dates
    const firstPhotoDate = photosWithDates.length > 0 ? photosWithDates[0].date : null;
    const latestPhotoDate = photosWithDates.length > 0 ? photosWithDates[photosWithDates.length - 1].date : null;
    
    let timeSpan: { days: number; hours: number; minutes: number } | null = null;
  ...
  const formatTimeGap = (gap: { days: number; hours: number; minutes: number } | null): string => {
    if (!gap) return 'N/A';
    const parts: string[] = [];
    if (gap.days > 0) parts.push(`${gap.days} day${gap.days !== 1 ? 's' : ''}`);
    if (gap.hours > 0) parts.push(`${gap.hours} hour${gap.hours !== 1 ? 's' : ''}`);
    if (gap.minutes > 0 || parts.length === 0) parts.push(`${gap.minutes} minute${gap.minutes !== 1 ? 's' : ''}`);
    return parts.join(', ');
  };

  const displayedRooms = expandedRooms ? stats.photosPerRoom : stats.photosPerRoom.slice(0, 10);
  const displayedUsers = expandedUsers ? stats.photosPerUser : stats.photosPerUser.slice(0, 10);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Statistics for photos and feed photos{accountId ? ` (Account: ${accountId})` : ''}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="p-4 border rounded-lg">
                ...
                <div className="text-2xl font-bold">{photos.length + feedPhotos.length}</div>
                <div className="text-xs text-muted-foreground mt-1">
                  {photos.length} photos, {feedPhotos.length} feed photos
                </div>
              </div>
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Unique Rooms</div>
                <div className="text-2xl font-bold">{stats.totalUniqueRooms}</div>
              </div>
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Unique Users Tagged</div>
                <div className="text-2xl font-bold">{stats.totalUniqueUsers}</div>
              </div>
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Total Cheers</div>
                <div className="text-2xl font-bold">{stats.totalCheers.toLocaleString()}</div>
              </div>
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Largest Time Gap</div>
                <div className="text-lg font-semibold">{formatTimeGap(stats.largestTimeGap)}</div>
                {stats.largestTimeGap?.photo1Date && stats.largestTimeGap?.photo2Date && (
                  <div className="text-xs text-muted-foreground mt-1">
              ...
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground">Smallest Time Gap</div>
                <div className="text-lg font-semibold">{formatTimeGap(stats.smallestTimeGap)}</div>
                {stats.smallestTimeGap?.photo1Date && stats.smallestTimeGap?.photo2Date && (
                  <div className="text-xs text-muted-foreground mt-1">
            ...
            {/* Date Range */}
            {stats.firstPhotoDate && stats.latestPhotoDate && stats.timeSpan && (
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-muted-foreground mb-2">Photo Timeline</div>
                <div className="space-y-1">
                  <div className="flex justify-between">
                    <span className="text-sm">First Photo:</span>
                    <span className="font-semibold">{format(stats.firstPhotoDate, 'MMMM d, yyyy')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">Latest Photo:</span>
                    <span className="font-semibold">{format(stats.latestPhotoDate, 'MMMM d, yyyy')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">Time Span:</span>
                    <span className="font-semibold">{formatTimeGap(stats.timeSpan)}</span>
                  </div>
                </div>
              </div>
            )}
                    ...
                    >
                      {expandedRooms ? (
                        ...
                        <>
                          ...
                          Show All ({stats.photosPerRoom.length})
                        </>
                      )}
                    </button>
                  )}
                </div>
                <ResponsiveContainer width="100%" height={Math.max(300, displayedRooms.length * 30)}>
                  <BarChart
                    data={displayedRooms}
                    layout="vertical"
                  ...
                  {stats.photosPerUser.length > 10 && (
                    ...
                    >
                      {expandedUsers ? (
                        ...
                        <>
                          <ChevronDown className="h-4 w-4" />
                          Show All ({stats.photosPerUser.length})
                        </>
                      )}
                    </button>
                  )}
                </div>
                <ResponsiveContainer width="100%" height={Math.max(300, displayedUsers.length * 30)}>
                  <BarChart
                    data={displayedUsers}
                    layout="vertical"
                    margin={{ top: 5, right: 30, left: 100, bottom: 5 }}
